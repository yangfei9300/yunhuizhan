		...
		<view class="colonn">
			...
			>  
				<view class="roww w-650 fs-30 center_center">
					...
					<block v-if="exType=='2'">  
						<view class="w-590 fs-40">{{index+1}}.{{item.content}}({{item.qType==1?'单选题':'多选题'}})</view>
					</block>
					<block v-else>
				...
				<view class="colonn m-top-15" style="border-top: 1px solid #c9c9c9;">
					...
					>
						...
						<view class="w-550 lh-40 fs-40">{{item1.txt}}</view>
						<view class="allline"></view>
					</view>
				</view>
...
export default {
	...
	methods: {
		...
		isZuodaNow(){
			var zhanhuiwenjuan=this.zhanhuiwenjuan
			var indexNow=this.indexNow;
			if(zhanhuiwenjuan[indexNow].seltxt.length<=0){
				this.$tools.showToast("请作答完再进行下一步");
				return false;
			}
			return true;
		},
